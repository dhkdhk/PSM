...
class CustomDataset(Dataset):
    ...
    def __init__(self, data, labels, transform=None, target_transform=None, two_crop=False):
        ...
        # shuffle the dataset
        idx = np.random.permutation(data.shape[0])             # np.random.permutation 随机排列函数
...
def random_split(data, labels, n_classes, n_samples_per_class):
    ...
    return {'train': torch.from_numpy(np.stack(train_x)),              # np.stack()
...
def sample_weights(labels):
    ...
    # list(map(int, labels)) 返回list形式的labels
...
def experiment_config(parser, args):
    ...
    run_dir = '/mnt/data/donghengkui/experiments/moco'
